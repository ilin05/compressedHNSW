...
namespace camel {
    ...
    long long DoubleCamelDecoder::integer_decode() {
        long long diff = in->readLong(2);
        if (diff <= 2) {
            return previous_integer + diff - 1;
        }
        long long sign = in->readBoolean() ? 1 : -1;
        bool gt8 = in->readBoolean();
        long long magnitude = in->readLong(gt8 ? 16 : 3);
    ...
    double DoubleCamelDecoder::decimal_decode() {
        ...
        if (c1 && l > 0) {
            unsigned long long center = static_cast<unsigned long long>(in->readLong(l));
            vd = center << (52 - l);
        }
