...
namespace camel {
    ...
    DoubleCamelEncoder::DoubleCamelEncoder(std::shared_ptr<utils::BaseStreamWriter> sharedOut)
        : Encoder(std::move(sharedOut)) {}

    DoubleCamelEncoder::DoubleCamelEncoder(std::shared_ptr<utils::BaseStreamWriter> sharedOut, const std::string& config)
