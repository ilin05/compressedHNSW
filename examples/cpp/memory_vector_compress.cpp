...
int main() {
    try {
        ...
        for (const std::string& algorithm : kAlgorithms) {
            ...
            // Use MemoryStreamWriter
            std::vector<char> memoryBuffer;
            auto sharedWriter = std::make_shared<utils::MemoryStreamWriter>(&memoryBuffer);
            ...
            for (int row = 0; row < rows; ++row) {
                ...
                int leftBits = sharedWriter->getLeftBits();
            ...
            // Use MemoryBlockStreamReader
            // Cast vector<char> data to const unsigned char*
            const unsigned char* dataPtr = reinterpret_cast<const unsigned char*>(memoryBuffer.data());
            std::shared_ptr<utils::MemoryBlockStreamReader> reader = std::make_shared<utils::MemoryBlockStreamReader>(dataPtr);
