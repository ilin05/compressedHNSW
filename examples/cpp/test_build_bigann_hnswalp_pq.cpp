...
int main(int argc, char** argv) {
    ...
    std::ofstream csv("bigann_hnswalp_pq_build_results.csv");
    ...
    for (size_t m : subsets) {
        ...
        std::cout << "\n=== Building HNSWALP_PQ for " << bigann_test_utils::subset_label(m) << " ===" << std::endl;

        int dim = 0;
        L2SpaceDouble l2space(128);
        HierarchicalNSWALPSIMPLIFIEDPQ<double> index(&l2space, vec_count, M, ef_construction);
        ...
        std::cout << "  build=" << build_s << "s compress=" << comp_s << "s" << std::endl;
    ...
    std::cout << "\nSaved CSV: bigann_hnswalp_pq_build_results.csv" << std::endl;
    return 0;
}
