...
TestResult test_build_index(const std::string& dataset_name, const std::string& base_dir) {
    ...
    HierarchicalNSW<double>* appr_alg = 
        new HierarchicalNSW<double>(&l2space, num_vectors, M, efConstruction);
    ...
    // 保存索引
    std::string index_path = dataset_name + "_hnsw.bin";
...
int main(int argc, char** argv) {
    ...
    write_results_to_csv("hnsw_build_results.csv", all_results);
