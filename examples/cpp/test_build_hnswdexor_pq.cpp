...
TestResult test_build_index(const std::string& dataset_name, const std::string& base_dir) {
    ...
    size_t cache_max_size = num_vectors / 100; // 设置缓存大小为总数据的1%
    ...
    HierarchicalNSWCABLEANNPQ<double>* appr_alg = 
        new HierarchicalNSWCABLEANNPQ<double>(&l2space, num_vectors, "DeXOR", M, efConstruction, true, cache_max_size);
    ...
    // 保存索引
    std::string index_path = dataset_name + "_hnswcableann_pq.bin";
...
int main(int argc, char** argv) {
    ...
    // 输出到 CSV 文件
    write_results_to_csv("hnswcab_leann_build_compression_results.csv", all_results);
