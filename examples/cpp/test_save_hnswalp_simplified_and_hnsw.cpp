...
std::vector<double> test_save_hnswalp_simplified(std::string data_path, std::string file_name) {
    std::string file_path = data_path + "/" + file_name + ".csv";
    std::vector<std::vector<double>> data = data_loader::loadData(file_path);
    std::vector<double> results;
    if (data.empty()) {
    ...
    // Initing index
    hnswlib::L2SpaceDouble space(dim);
    hnswlib::HierarchicalNSWALPSIMPLIFIED<double>* alg_hnsw = new hnswlib::HierarchicalNSWALPSIMPLIFIED<double>(&space, max_elements, M, ef_construction);
...
std::vector<double> test_load_hnswalp_simplified(std::string data_path, std::string file_name) {
    std::string file_path = data_path + "/" + file_name + ".csv";
    std::vector<double> results;
    int rows = 0, cols = 0;
    double* data_ptr = data_loader::loadDataForSearch(file_path, rows, cols);
    ...
    hnswlib::L2SpaceDouble space(dim);
    std::string hnswalp_leann_path = "storage/" + file_name + "_hnswalp_simplified.bin";
    hnswlib::HierarchicalNSWALPSIMPLIFIED<double>* alg_hnsw = new hnswlib::HierarchicalNSWALPSIMPLIFIED<double>(&space, hnswalp_leann_path);
...
void collect_save_hnswalp_simplified_results(){
    for(const auto& file_name : file_names){
        std::cout << "Processing file: " << file_name << std::endl;
        std::vector<double> save_results = test_save_hnswalp_simplified("../datasets/", file_name);
...
void collect_load_hnswalp_simplified_results(){
    for(const auto& file_name : file_names){
        std::cout << "Processing file: " << file_name << std::endl;
        std::vector<double> load_results = test_load_hnswalp_simplified("../datasets/", file_name);
...
int main() {
    ...
    collect_save_hnswalp_simplified_results();
    collect_save_hnsw_results();
    collect_load_hnswalp_simplified_results();
    collect_load_hnsw_results();
    // collect_analyze_hnsw_graph_results();

    write_results_to_csv("hnswalp_simplified_hnsw_test_results.csv");
    return 0;
}
