...
int main(int argc, char** argv) {
    ...
    std::ofstream csv("bigann_hnsw_search_results_recall@10.csv");
    ...
    for (size_t m : subsets) {
        ...
        if (!bigann_test_utils::load_gt_ivecs_topk(gt_file, qsize, k, gt)) {
            std::cerr << "Skip " << m << "M due to GT load failure." << std::endl;
            continue;
        }

        std::cout << "\n=== Searching HNSW for " << bigann_test_utils::subset_label(m) << " ===" << std::endl;
        HierarchicalNSW<double> index(&l2space, index_file, false);
        ...
        for (size_t ef : efs) {
            ...
            std::cout << "  ef=" << ef << " recall=" << std::fixed << std::setprecision(4)
                      << recall << " time=" << std::setprecision(3) << time_ms << " ms" << std::endl;
    ...
    delete[] queries;
    std::cout << "\nSaved CSV: bigann_hnsw_search_results_recall@10.csv" << std::endl;
    return 0;
}
