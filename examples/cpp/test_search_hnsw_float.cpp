...
void test_search_dataset(const std::string& dataset_name, const std::string& base_dir, std::ofstream& csv_file) {
    ...
    std::string index_path = dataset_name + "_train.fvecs_hnsw_float.bin";
    ...
    float* massQ = load_fvecs(query_file, qsize, qdim);
    if (!massQ) return;
...
int main(int argc, char** argv) {
    ...
    std::string csv_file_path = "hnsw_float_search_recall_results.csv";
    ...
    csv_file.close();
    cout << "\nAll search tests (Original HNSW) completed. Results written to " << csv_file_path << endl;
    return 0;
}