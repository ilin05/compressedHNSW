...
class HierarchicalNSWALP : public AlgorithmInterface<dist_t> {
 public:
    ...
    void alp_encode_vector(const double* data, size_t dim, utils::MemoryStreamWriter& writer) const {
        ...
        // Heuristic: Try exponents 0 to 16
        // We prefer smaller exponents if they work (lossless)
        // Or we pick the one that results in smallest bit width for deltas
        
        // For simplicity in this implementation, we look for the first exponent that makes all values integers (lossless).
        // If none found, we default to a high precision (e.g. 16) or fallback to raw (not implemented here).
        // Let's assume we want to preserve up to some precision.
        ...
        for (int exp = 0; exp <= 16; ++exp) {
            ...
            for (size_t i = 0; i < dim; ++i) {
                double val = data[i] * factor;
                if (std::abs(val - std::round(val)) > 1e-4) { // Tolerance
                    lossless = false;
                    break;
                }
            }
            if (lossless) {
                selected_exp = exp;
                break; // Found smallest sufficient exponent
            }
        }
        
        if (selected_exp == -1) selected_exp = 16; // Default to high precision if no perfect match
        
        writer.write(selected_exp, 8); // Write exponent (1 byte)
        
        double factor = std::pow(10.0, selected_exp);
        
        // 2. Calculate Deltas and Bit Width
        std::vector<int64_t> deltas(dim);
        int64_t prev = 0;
        uint64_t max_zigzag = 0;
        
        for (size_t i = 0; i < dim; ++i) {
            int64_t val = (int64_t)std::round(data[i] * factor);
            deltas[i] = val - prev;
            prev = val;
            
            uint64_t zz = zigzag_encode(deltas[i]);
            if (zz > max_zigzag) max_zigzag = zz;
        }
        
        int bit_width = get_bit_width(max_zigzag);
        writer.write(bit_width, 8); // Write bit width (1 byte)
        
        // 3. Write Deltas
        for (size_t i = 0; i < dim; ++i) {
            writer.write((long long)zigzag_encode(deltas[i]), bit_width);
        }
    }
    ...
    void alp_decode_vector(utils::MemoryStreamReader& reader, std::vector<double>& result) const {
        ...
        for (size_t i = 0; i < dim; ++i) {
            uint64_t zz = reader.readLong(bit_width);
