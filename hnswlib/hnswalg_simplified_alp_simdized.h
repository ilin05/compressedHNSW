...
class HierarchicalNSWALPSIMPLIFIEDSIMD : public AlgorithmInterface<dist_t> {
 public:
    ...
    void alp_encode_vector(const double* data, size_t dim, utils::MemoryStreamWriter& writer) const {
        ...
        // Force alignment after writing vector?
        // writer.align(); // Optional, but good practice if we want next vector to start aligned
    }
    ...
    void alp_decode_vector(utils::MemoryStreamReader& reader, std::vector<double>& result) const {
        ...
        std::vector<int64_t> deltas(dim);

        // Align reader to byte boundary
        const unsigned char* aligned_ptr = reader.get_current_ptr_and_advance_bytes(0);
        
        // Manual Unpacking from aligned_ptr
        // Note: The writer.write() might write bits across byte boundaries.
        // However, we called writer.align() BEFORE writing deltas.
        // So the first delta starts at aligned_ptr.
        
        // We need to implement a fast unpacker here.
        // Since we are inside the header, we can define a lambda or helper.
        // For now, let's use a robust bit reader loop using 64-bit buffer.
        
        size_t total_bits = dim * bit_width;
        size_t byte_idx = 0;
        int bit_idx = 0; // 0..7
        
        // Optimization: Use a local buffer/prefetch if possible.
        // Or simpler: Reuse reader.readLong but that handles unaligned.
        // Since we are aligned now, we can optimize.
        
        // But Reader.readLong handles alignment logic internally (leftBits).
        // Since we called get_current_ptr_and_advance_bytes(0) which aligns the reader (sets leftBits=8),
        // we can assume we are at byte boundary.
        
        // However, MemoryStreamReader.readLong() updates 'pointer' and 'leftBits'.
        // If we bypass it, we must update them or advance 'bytes' at the end.
        
        // Let's consume all bytes at once.
        size_t total_bytes = (total_bits + 7) / 8;
        const unsigned char* src = reader.get_current_ptr_and_advance_bytes(total_bytes);
        
        if (bit_width == 0) {
            std::fill(deltas.begin(), deltas.end(), 0);
        } else {
            // Optimized Scalar Unpacker matching MemoryStreamWriter bit layout
            // This replaces the individual reader.readLong() calls to avoid overhead
            size_t ptr = 0; // offset into src
            int lb = 8;     // bits left in current byte
            
            // Prefetch bit_width to register
            int bw = bit_width;
            ...
            for(size_t i=0; i<dim; ++i) {
                ...
                int size = bw;
                while (size > 0) {
                    int bitsToRead = (size < lb) ? size : lb;
                    lb -= bitsToRead;
                    
                    // Extract bits
                    unsigned int mask = (1u << bitsToRead) - 1u;
                    unsigned int chunk = (src[ptr] >> lb) & mask;
        ...
        // SIMD Decode
        AlpSIMDHelper::simd_add_and_scale(deltas.data(), (double)min_val, factor, result.data(), dim);
    }

    HierarchicalNSWALPSIMPLIFIEDSIMD(SpaceInterface<dist_t> *s) {
    }


    HierarchicalNSWALPSIMPLIFIEDSIMD(
    ...
    HierarchicalNSWALPSIMPLIFIEDSIMD(
        ...
        if (linkLists_ == nullptr)
            throw std::runtime_error("Not enough memory: HierarchicalNSWALPSIMPLIFIEDSIMD failed to allocate linklists");
