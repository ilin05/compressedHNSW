...
test_approx(
    ...
    size_t vecsize,
    size_t qsize,
    HierarchicalNSW<double> &appr_alg,
    size_t vecdim,
    vector<std::priority_queue<std::pair<double, labeltype>>> &answers,
    size_t k) {
    size_t correct = 0;
...
test_vs_recall(
    ...
    std::string csv_file_path = "sift1b_hnsw_recall_results.csv";
...
void sift_test1B_double_hnsw() {
    ...
    if (exists_test(path_index)) {
        ...
        appr_alg = new HierarchicalNSW<double>(&l2space, path_index, false);
        cout << "Actual memory usage: " << getCurrentRSS() / 1000000 << " Mb " << endl;
    } else {
        cout << "Building index:" << endl;
        appr_alg = new HierarchicalNSW<double>(&l2space, vecsize, M, efConstruction);
