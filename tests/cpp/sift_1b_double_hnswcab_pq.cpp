...
test_vs_recall(
    ...
    std::string csv_file_path = "sift1b_hnswcab_pq_recall_results.csv";
...
void sift_test1B_double_hnswcab_pq() {
    ...
    snprintf(path_index, sizeof(path_index), "sift1b_hnswcab_pq_%dm_ef_%d_M_%d.bin", subset_size_milllions, efConstruction, M);
    ...
    if (exists_test(path_index)) {
        ...
        appr_alg = new HierarchicalNSWCABLEANNPQ<double>(&l2space, path_index, true, cache_size, false, vecsize);
        cout << "Actual memory usage: " << getCurrentRSS() / 1000000 << " Mb " << endl;
    } else {
        cout << "Building index:" << endl;
        appr_alg = new HierarchicalNSWCABLEANNPQ<double>(&l2space, vecsize, "DeXOR", M, efConstruction, true);
